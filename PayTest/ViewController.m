...
- (IBAction)AliPayButtonAction:(id)sender {
    ...
    NSLog(@"--------------");
    [[SAPlatformPayManager sharePayManager] aliPayOrder:orderMessage scheme:@"com.zhangjiong.alipay.payTest.scheme" responeBlock:^(NSInteger responeCode, NSString *responeMsg) {
        NSLog(@"----%ld------%@-----",(long)responeCode,responeMsg);
    }];
}
